...
function setupCountryFactsModal() {
    ...
    if (!document.getElementById('countryFactsModal')) {
        ...
        modal.className = 'country-facts-modal'; // Use a class for styling
        modal.innerHTML = `
  ...
  background: #2563eb;
    ...
    if (learnMoreLink && regionDetail) {
        learnMoreLink.addEventListener('click', async (e) => {
            ...
            try {
                const facts = await getInterestingFacts(code);
                // Make API data bold by wrapping the dynamic values in <strong>
                factsList.innerHTML = facts.map(fact => {
                    // Replace dynamic values (between "that ", "is ", "speak ", "currency is the ") with <strong>
                    return `<li>${fact.replace(/(that |is |speak |currency is the )([^.,?]+)/g, (m, p1, p2) => p1 + '<strong>' + sanitizeText(p2.trim()) + '</strong>')}</li>`;
                }).join('');
            } catch {
